...
TeslaApp.factory('searchFactory', ['fdaApiService', 'rxNormApiService', 'backendApiService', 'teslaFactory', '$q',
    function (fdaApiService, rxNormApiService, backendApiService, teslaFactory, $q) {
        ...
        return {
             ...
             * @param callback
             */
            getDrugEventsByBrandName: function(brandName, callback){
                var drugData = {};
                var drugEventSearchString = "brand_name:" + brandName;

                var drugsEventsPromise = fdaApiService.getDrugEvent(fdaApiService.queryBuilder()
                    .searchString(drugEventSearchString)
                    .setCount('patient.reaction.reactionmeddrapt.exact'));

                drugsEventsPromise.then(function (eventResult) {
